#!/usr/bin/env bash
# init-c-to-rust-project.sh — generate a Rust migration project for a C source tree.
set -euo pipefail

SOURCE_PATH="${1:?usage: $0 <source_path> <target_path> [project_name]}"
TARGET_PATH="${2:?usage: $0 <source_path> <target_path> [project_name]}"
PROJECT_NAME="${3:-$(basename "$TARGET_PATH")}"
CRATE_NAME="$(printf '%s' "$PROJECT_NAME" | tr '[:upper:]-' '[:lower:]_' | sed 's/[^a-z0-9_]/_/g')"
...
if [ ! -d "$SOURCE_PATH/src" ]; then
  echo "ERROR: source project must contain src/: $SOURCE_PATH/src" >&2
  exit 1
fi
...
deliverables:
  - "Cargo project named $CRATE_NAME"
